...
class GuidedTripsController {
  constructor(supabaseClient) {
    this.tripRepo = new GuidedTripsRepository(supabaseClient);
  }

  // READ - Get all guided trips
  async getAllGuidedTrips(req, res) {
    try {
      ...
      if (min_seats) {
        trips = await this.tripRepo.findTripsByAvailableSeats(parseInt(min_seats));
      } else {
        trips = await this.tripRepo.getAllTrips();
      }
  ...
  async getGuidedTripById(req, res) {
    try {
      const { id } = req.params;
...
export default GuidedTripsController;
