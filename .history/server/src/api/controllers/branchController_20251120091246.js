 ...
 *              Routes requests to BranchesRepository and manages business logic
 * 
 * @author      Abderahim
 * @version     1.0.0
 * @date        2025-11-18
...
class BranchController {
  constructor(supabaseClient) {
    this.branchRepo = new BranchesRepository(supabaseClient);
  }
  ...
  async createBranch(req, res) {
    try {
      ...
      const existingBranches = await this.branchRepo.getAllBranches();
  ...
  async getAllBranches(req, res) {
    try {
      ...
      if (city) {
        branches = await this.branchRepo.findBranchesByCity(city);
      } else if (active === 'true') {
        branches = await this.branchRepo.getActiveBranches();
      } else {
        branches = await this.branchRepo.getAllBranches();
      }
  ...
  async updateBranch(req, res) {
    try {
      ...
      const branch = await this.branchRepo.updateBranch(id, {
        branch_name,
        branch_address,
        branch_city,
        phone,
  ...
  async deleteBranch(req, res) {
    try {
      ...
      await this.branchRepo.deleteBranch(id);
